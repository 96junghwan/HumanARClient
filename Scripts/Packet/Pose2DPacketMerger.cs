...
{
    public class Pose2DPacketMerger
    {
        // 내부 클래스
        class MergedPacket
        {
        ...
        // 생성자
        public Pose2DPacketMerger(int listMax, PostProcessOptionModel postProcessOptionModel, Queue<DetectHumanJointResultMsg> humanPoseMsgQ)
        {
        ...
        // 수신한 관절 패킷 하나 넣는 함수
        public void PutPacket(Response2DPosePacketStruct packet, byte[] jointByte)
        {
            ...
            if (index != -1)
            {
                ...
                if ((packet.order & (int)Order.First) == (int)Order.First)
                {
                    ...
                    packetList[index].jointNumbers = packet.jointNumbers;
                    packetList[index].jointByte = new byte[packet.jointWholeSize];
                }

                // 마스크 데이터 복사하여 저장
                Buffer.BlockCopy(jointByte, 0, packetList[index].jointByte, (int)packet.offset, packet.dataSize);
        ...
        public void SendMsg(int index)
        {
            ...
            {
                jointList = JointParser.Bytes2HumanJointList(
                    packetList[index].jointByte, 
                    packetList[index].jointNumbers, 
            ...
            // NetworkManger에 전달
            this.humanPoseMsgQ.Enqueue(new DetectHumanJointResultMsg(packetList[index].frameID, jointList));
